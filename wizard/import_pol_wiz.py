...
class ImportPOLWiz(models.TransientModel):
    _name = 'import.pol.wiz'
    _description = 'Import Purchase Order Line Wizard'

    po_id = fields.Many2one("purchase.order", string="Purchase Order", default=lambda self: self._default_po_id())
    vendor_name = fields.Many2one('res.partner', related='po_id.partner_id', string="Partner Name")
    ...
    @api.model
    def _default_po_id(self):
        return self.env.context.get('active_id') if self.env.context.get('active_model') == 'purchase.order' else None

    def download_sample_file(self):
        return self.env.ref("sha_import_order_lines.sample_pol_xlsx").report_action(self)
    ...
    def import_xlsx_file(self):
        ...
        for row in range(1, sheet.nrows):
            ...
            if not product:
                message_lines.append(f"<p><span style='display: inline-block; width: 15px; height: 15px; background-color: red; border-radius: 50%; color: white; text-align: center; line-height: 15px;'>&#10006;</span> Line {row + 1}: Not Found '{product_name}'</p>")
                continue
            ...
            message_lines.append(f"<p><span style='display: inline-block; width: 15px; height: 15px; background-color: green; border-radius: 50%; color: white; text-align: center; line-height: 15px;'>&#10004;</span> Line {row + 1}: Imported '{product_name}'</p>")

        if order_lines:
            self.po_id.write({"order_line": order_lines})
